...
echo "n" | pio template get harshmathur1990/template-scala-parallel-universal-recommendation JuggernautRecommendor --name "none" --package "none" --email "none"
cd JuggernautRecommendor
...
pio app new JuggernautRecommendor > log.txt
...
sed -i "s|INVALID_APP_NAME|MyApp1|" /quickstartapp/JuggernautRecommendor/engine.json
...
echo "You may now deploy engine by running cd /quickstartapp/JuggernautRecommendor && pio deploy --ip 0.0.0.0"
